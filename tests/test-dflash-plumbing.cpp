...
int main(int argc, char ** argv) {
    ...
    ok &= expect(context_cpp.find("dflash_gpu_capture_ready = (model.n_devices() <= 1 || llama_dflash_allow_multi_gpu_tape()) && dflash_capture->gpu_capture_enabled;") != std::string::npos, "DFlash graph capture must be gated to single-GPU or multi-GPU tape mode and explicit GPU capture policy");
